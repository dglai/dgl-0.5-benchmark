...
class GraphSAGE(nn.Module):
    def __init__(self,
        ...
        # input layer
        self.layers.append(SAGEConv(in_feats, hidden_feats, 'mean', bias=False))
        self.bns.append(nn.BatchNorm1d(hidden_feats))
        # hidden layers
        for _ in range(num_layers - 2):
            self.layers.append(SAGEConv(hidden_feats, hidden_feats, 'mean', bias=False))
            self.bns.append(nn.BatchNorm1d(hidden_feats))
        # output layer
        self.layers.append(SAGEConv(hidden_feats, out_feats, 'mean', bias=False))
