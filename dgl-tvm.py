...
def bench_spmm(g, target, ctx, th_ctx, generic=False):
    ...
    def measure_time(binary_op, reduce_op, 
                     node_feat_shape, edge_feat_shape, feat_type, 
                     num_warmup, num_runs, verify=False):
        use_bcast, lhs_len, rhs_len, out_len, _, lhs_off, rhs_off = \
            calc_bcast(binary_op, np.zeros((1,)+node_feat_shape), np.zeros((1,)+edge_feat_shape))
        if generic:
        ...
        if use_node:
            node_feat = th.rand((num_cols, lhs_len), dtype=th_dtype_mapping[feat_type], device=th_ctx)
            tvm_node_feat = tvm.nd.from_dlpack(th.utils.dlpack.to_dlpack(node_feat))
            f_input.append(tvm_node_feat)
        if use_edge:
            edge_feat = th.rand((nnz, rhs_len), dtype=th_dtype_mapping[feat_type], device=th_ctx)
            tvm_edge_feat = tvm.nd.from_dlpack(th.utils.dlpack.to_dlpack(edge_feat))
            f_input.append(tvm_edge_feat)
        if use_bcast:
        ...
        if verify:
            ...
            print('binary_op: {}\treduce_op:{}\tnode_feat_shape: {}\tedge_feat_shape: {}\telpased time: {:.3e}s'
                .format(binary_op, reduce_op, node_feat_shape, edge_feat_shape, tcost))
        # except Exception as e:
        #     print(e)
    shapes = [(2 ** x,) for x in range(8)]
    for shape in shapes:
        measure_time('copy_u', 'sum', shape, shape, 'float32', n_cold_start, num_runs)
    for shape in shapes:
...
def bench_sddmm(g, target, ctx, th_ctx, generic=False):
    ...
    def measure_time(binary_op, src_feat_shape, dst_feat_shape, 
        ...
        try:
            ...
            if verify:
                f(*f_input)
                lhs = src_feat[adj_scipy_coo.row]
                rhs = dst_feat[adj_scipy_coo.col]
                if binary_op != 'dot':
...
if __name__ == '__main__':
    ...
    for dataset in ['arxiv', 'reddit', 'proteins']:
        ...
        # SDDMM
        # bench_sddmm(g, target, tvm_ctx, ctx, generic=False)