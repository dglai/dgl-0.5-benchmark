...
class GCNConv(nn.Module):
    ...
    def forward(self, graph, feat, bond):
        ...
        graph.ndata['x'] = x
        graph.edata['w'] = self.bond_encoder(bond)
...
class GCN(nn.Module):
    ...
    def forward(self, g, atom, bond):
        x = self.atom_encoder(atom)
        for i, layer in enumerate(self.layers[:-1]):
            x = layer(g, x, bond)
            x = self.bns[i](x)
            x = F.relu(x)
            x = self.dropout(x)
        x = self.layers[-1](g, x, bond)
...
def train(model, device, train_loader, optimizer):
    ...
    for i, (batched_graph, labels) in enumerate(train_iter):
        ...
        # train
        optimizer.zero_grad()
        atom = batched_graph.ndata['feat']
        bond = batched_graph.edata['feat']
        out = model(batched_graph, atom, bond)
...
def test(model, device, loader, evaluator):
    ...
    for step, (batched_graph, labels) in enumerate(tqdm(loader, desc='Iter')):
        ...
        atom = batched_graph.ndata['feat']
        bond = batched_graph.edata['feat']
        pred = model(batched_graph, atom, bond)
